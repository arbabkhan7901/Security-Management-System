...
<head>
	 ...
	 function Main(){
			$("#btn1").click(function(){
				if(flag == false){
					var per = $("#role").val();
					var des = $("#des").val();
					var data = {"action": "insertper", "per": per, "des": des};
					var sett = {
						type: "POST",
						dataType: "json",
						url: "perapi.php",
	...
	function loadTable(){
		var $table = $("#div2");
		$table.empty();
		$table.append("<tr><td> Permission id </td><td> Permission</td> <td> Description </td> <td> Created on </td><td> Created by </td><td> Edit </td><td> Delete </td></tr>");
		 var data = {"action": "getallpers"};
		 var setti = {
				type: "POST",
				dataType: "json",
				url: "perapi.php",
				...
				success:function(res){
					for (var i = 0; i < res.data.length; i++){
						...
						var $button = $("<button>");
						$button.text("Edit");
						$button.attr("id", obj.permissionid);
						$td.append($button);
						$tr.append($td);
						...
						var $button1 = $("<button>");
						$button1.text("Delete");
						$button1.attr("id", obj.permissionid);
						$td.append($button1);
						$tr.append($td);
                        ...
                        if ($isConfirm == true) {
							...
							var id = $(this).attr("id");
							var data = {"action": "deleteper", "id": id};
							var set = {
								type: "POST",
								dataType: "json",
								url: "perapi.php",
