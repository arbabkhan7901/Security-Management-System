...
<head>
    ...
    <script>
        function Main() {
            ...
            var u_arr = SecurityManager.GetAllPermissions();
            for (var i = 0; i < u_arr.length; i++) {
                ...
                for (var j = 0; j < 5; j++) {
                    ...
                    if (j == 2) {
                        newText = document.createTextNode(u_arr[i].des);
                        newCell.style.border = '1px solid black';
                    }
                    if (j == 3) {
	                    ...
	                    var edit_user = SecurityManager.GetPermissionById(this.id);
                            document.getElementById("role").value = edit_user.role;
                            document.getElementById("des").value = edit_user.des;
                            var sBtn = document.getElementById("btn1");
                            sBtn.onclick = function () {
                                edit_user.role = document.getElementById("role").value;
                                edit_user.des = document.getElementById("des").value;
                                var user_arr = SecurityManager.GetAllPermissions();
                                var flag1 = true;
                               
                                    SecurityManager.SavePermission(edit_user, function () { alert("user has been edited") }, function () { alert("user has not been edited") });
                                window.location.href = "PermissionMangement.html";
                            };
                        };
                        newCell.style.border = '1px solid black';
                    }
                    if (j == 4) {
                        ...
                        newText.onclick = function () {
                            ...
                            if (agree) {
                                SecurityManager.DeletePermission(this.id, function () { alert("Record Deleted"); }, function () { alert("Record can not deleted"); });
            ...
            save_btn.onclick = function () {
                ...
                var flag = true;
                if (!userObj.role || !userObj.des) {
                    alert("fill all the fields");
                    flag = false;
                }
                var user_arr = SecurityManager.GetAllPermissions();
                for (var i in user_arr) {
                    if (user_arr[i].role == role) {
                        alert("User already exists");
                        flag = false;
                    }
                }
                if (flag == true) {
                    SecurityManager.SavePermission(userObj, function () { alert("user has been created") }, function () { alert("user has not been created") });
                    ...
                    var t3 = document.createTextNode(userObj.des);
...
<body onload="Main();">
    ...
    <div class="div1">
        ...
        <form>
            <br><input id="role" type="text" class="box" placeholder="Permission Name" required>
            <br><br>
            <input id="des" type="text" class="box" placeholder="Description" required>
            <br><br>
