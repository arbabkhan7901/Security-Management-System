...
<head>
    ...
    <script>
        function Main() {
            ...
            var u_arr = SecurityManager.GetAllRolePermissions();
            for (var i = 0; i < u_arr.length; i++) {
                ...
                for (var j = 0; j < 5; j++) {
                    ...
                    if (j == 2) {
                        newText = document.createTextNode(u_arr[i].per);
                        newCell.style.border = '1px solid black';
                    }
                    if (j == 3) {
                        ...
                        newText.onclick = function () {
                            var edit_user = SecurityManager.GetRolePermissionById(this.id);
                            document.getElementById("role").value = edit_user.role;
                            document.getElementById("permission").value = edit_user.per;
                            var sBtn = document.getElementById("btn1");
                            sBtn.onclick = function () {
                                edit_user.role = document.getElementById("role").value;
                                edit_user.per = document.getElementById("permission").value;
                                var user_arr = SecurityManager.GetAllRoles();
                                    SecurityManager.SaveRolePermission(edit_user, function () { alert("user has been edited") }, function () { alert("user has not been edited") });
                                window.location.href = "RolePermission.html";
                            };
                        };
                        newCell.style.border = '1px solid black';
                    }
                    if (j == 4) {
                        ...
                        newText.onclick = function () {
                            ...
                            if (agree) {
                                SecurityManager.DeleteRolePermission(this.id, function () { alert("Record Deleted"); }, function () { alert("Record can not deleted"); });
            ...
            for (var i = 0; i < per.length; i++) {
                var opt = document.createElement("option");
                opt.setAttribute("value", per[i].role);
                opt.innerText = per[i].role;
            ...
            save_btn.onclick = function () {
                ...
                var flag = true;
                if (!userObj.role || !userObj.per) {
                    alert("fill all the fields");
                    flag = false;
                }

                if (flag == true) {
                    SecurityManager.SaveRolePermission(userObj, function () { alert("user has been created") }, function () { alert("user has not been created") });
                    ...
                    var t3 = document.createTextNode(userObj.per);
...
<body onload="Main();">
    ...
    <div class="div1">
        ...
        <form>
            ...
            <br><br>
            Permission:<select name="" id="permission"></select>
            <br><br>
