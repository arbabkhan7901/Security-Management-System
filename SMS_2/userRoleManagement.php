...
<body >
    ...
    <div class="div1">
        ...
        <form>
			...
			?></select>
            <br><br>
            User:<select name="txt2" id="permission" required>
			<option value="" selected>--Select--</option>
			<?php
		...
		<?php
			...
			if($btn){
				$per = $_REQUEST['txt1'];
				$des = $_REQUEST['txt2'];
				$sql = "insert into user_role (userid, roleid)values ('$per','$des')";
				$result = mysqli_query($conn, $sql);
				if($result)
					echo "<script type='text/javascript'>alert('Data inserted successfully');</script>";
				else
					echo "<script type='text/javascript'>alert('Data doesn't inserted successfully');</script>";
				echo '<script type="text/javascript">window.location = "userRoleList.php" </script>';
			}
				}
				else{
					$sql = "SELECT * FROM user_role where id = '".$temp."'";
					$result = mysqli_query($conn, $sql);
					$row = mysqli_fetch_assoc($result);
					$temp1 = $row['userid'];
					$sql1 = "SELECT * FROM Users";
					$result1 = mysqli_query($conn, $sql1);
					$sql2 = "SELECT * FROM users where userid =  '".$temp1."'";
					...
					$result2 = mysqli_query($conn, $sql2);
					$sql3 = "SELECT * FROM roles where roleid =  '".$temp3."'";
			...
			<?php
				...
				if($temp1){
					$user = $_REQUEST['txt1'];
					$des = $_REQUEST['txt2'];
					$sql1 = "update user_role set userid = '$user', roleid = '$des' where id = '".$temp."'";
					$result1 = mysqli_query($conn, $sql1);
