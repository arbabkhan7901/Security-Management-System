...
color:#597A41;
	...
	<div id="bar">
      ...
      <?php
     require('conn.php');
      $result = mysqli_query($conn, "SELECT * FROM user_role");
      ?>
	  <form>
      ...
      <tbody>
        <?php
          while( $row = mysqli_fetch_assoc( $result ) ){
			  $var = $row['id'];
			  $role=$row['userid'];
			  $q="Select * from users where userid = '".$role."'";
			  $res = mysqli_query($conn, $q);
			  $temp = mysqli_fetch_assoc($res);	
			  $per=$row['roleid'];
			  $q1="Select * from roles where roleid = '".$per."'";
		 ...
		 if ($btn == true){
			 $id = $_REQUEST['edit'];
			 $_SESSION['unq4'] = $id;
			 echo '<script type="text/javascript">window.location = "UserRoleManagement.php" </script>';
		 }
		 if ($btn1){
			 $id = $_REQUEST['delete'];
			 $sql1 = "delete from user_role where id = '".$id."'";
	  ...
	  <input type='submit' name='btn3' value='Create new Role Permission' style= "position:absolute; left:500px;">
	  <?php
