		...
		<?php
			...
			if($btn){
				...
				while($res = mysqli_fetch_assoc($r)){
					if ($res['name'] == $role ){
						echo "<script type='text/javascript'>alert('Role already exits');</script>";
						$flag = false;
					}
				}
				if($flag){
					...
					$sql = "SELECT * FROM ROLES where roleid = '".$temp."'";
					$result = mysqli_query($conn, $sql);
					$row = mysqli_fetch_assoc($result);
			?>
			<form>
            <br>Role:<input id="role" name = "txt1" type="text" class="box" value="<?php echo( htmlspecialchars( $row['name'] ) ); ?>" required>
            <br><br>
			...
			<?php
				...
				if($temp1){
					$user = $_REQUEST['txt1'];
					$des = $_REQUEST['txt2'];
					$sql1 = "update roles set name = '$user', description = '$des' where roleid = '".$temp."'";
